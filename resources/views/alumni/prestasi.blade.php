...
<div class="prestasi-container">
    <form method="GET" action="{{ route('prestasiuser') }}" class="search-bar">
        <input type="text" name="search" class="form-control w-75" placeholder="Cari disini..." value="{{ request('search') }}">
        <button class="tambah"><a href="{{ route('tambahprestasiuser') }}">Tambah Prestasi</a></button>
    </form>
    ...
    <div>
        <div class="alumni-card">
            ...
            <div class="alumni-actions">
                <form id="hapus-form-{{ $item->id_prestasi }}" action="{{ route('dataprestasiuser.destroy', $item->id_prestasi) }}" method="POST">
                    @csrf
                    @method('DELETE')
                </form>
                <button class="btn btn-danger" onclick="hapusPrestasi('{{ $item->id_prestasi }}')"><i class="bi bi-trash3-fill"></i></button>
                <button class="btn btn-warning"><a href="{{ route('editprestasiuser', $item->id_prestasi) }}"><i class="bi bi-pencil-fill"></i></a></button>
            </div>
        </div>
    </div>
    @php $delay += 10; @endphp 
