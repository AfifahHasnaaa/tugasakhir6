...
<div class="inputalumni">
    <form id="formData" action="<?php echo e(url('alumni')); ?>" method="POST">
        <?php echo csrf_field(); ?>
        <div class="row">
            <!-- Kolom kiri: Profil, Domisili, dan Akademik -->
            <div class="col-md-6">
                <!-- Profil -->
                <div class="card mb-3">
            ...
            <!-- Kolom kanan: Akademik, Pendidikan Lanjutan, Pekerjaan, dan Kontak -->
            <div class="col-md-6">
                <!-- Akademik -->
                <div class="card mb-3">
...
<script>
    $(document).ready(function() {
        ...
        function sendData() {
            ...
            $.each(allInput, function(i, val) {
                dataForm[val['name']] = $(val).val(); // Ambil nilai dari input

                // Cek apakah input required dan kosong
                if ($(val).prop('required') && !dataForm[val['name']]) {
                    isValid = false; // Jika ada yang tidak valid
                    $(val).addClass('is-invalid'); // Menandai input yang tidak valid
                } else {
                    $(val).removeClass('is-invalid'); // Menghapus tanda jika valid
                }
            });
            ...
            // Validasi untuk alasan (jika status 'Tidak Bekerja' atau 'Belum Bekerja')
            if ((status === 'Tidak Bekerja' || status === 'Belum Bekerja') && !dataForm['alasan']) {
                isValid = false; // Jika alasan tidak diisi
                $('#alasanContainer input[name="alasan"]').addClass('is-invalid'); // Tandai input alasan
            } else {
                $('#alasanContainer input[name="alasan"]').removeClass('is-invalid'); // Hapus tanda jika valid
            }

            // Validasi untuk Pendidikan Lanjutan
            if (status === "Pendidikan Lanjut") {
                if (!dataForm['universitas']) {
                    isValid = false; // Jika universitas tidak diisi
                    $('#pendidikanContainer input[name="universitas"]').addClass('is-invalid'); // Tandai input universitas
                } else {
                    $('#pendidikanContainer input[name="universitas"]').removeClass('is-invalid'); // Hapus tanda jika valid
                }

                if (!dataForm['prodi']) {
                    isValid = false; // Jika prodi tidak diisi
                    $('#pendidikanContainer input[name="prodi"]').addClass('is-invalid'); // Tandai input prodi
                } else {
                    $('#pendidikanContainer input[name="prodi"]').removeClass('is-invalid'); // Hapus tanda jika valid
                }
            }
            ...
            if (status === "Bekerja" || status === "Wirausaha") {
                if (!dataForm['nama_perusahaan']) {
                    isValid = false; // Jika nama perusahaan tidak diisi
                    $('#pekerjaanContainer input[name="nama_perusahaan"]').addClass('is-invalid'); // Tandai input nama perusahaan
                } else {
                    $('#pekerjaanContainer input[name="nama_perusahaan"]').removeClass('is-invalid'); // Hapus tanda jika valid
                }

                if (!dataForm['bidang_pekerjaan']) {
                    isValid = false; // Jika bidang pekerjaan tidak diisi
                    $('#pekerjaanContainer input[name="bidang_pekerjaan"]').addClass('is-invalid'); // Tandai input bidang pekerjaan
                } else {
                    $('#pekerjaanContainer input[name="bidang_pekerjaan"]').removeClass('is-invalid'); // Hapus tanda jika valid
                }

                if (!dataForm['jabatan']) {
                    isValid = false; // Jika jabatan tidak diisi
                    $('#pekerjaanContainer input[name="jabatan"]').addClass('is-invalid'); // Tandai input jabatan
                } else {
                    $('#pekerjaanContainer input[name="jabatan"]').removeClass('is-invalid'); // Hapus tanda jika valid
                }

                if (!dataForm['penghasilan']) {
                    isValid = false; // Jika penghasilan tidak diisi
                    $('#pekerjaanContainer input[name="penghasilan"]').addClass('is-invalid'); // Tandai input penghasilan
                } else {
                    $('#pekerjaanContainer input[name="penghasilan"]').removeClass('is-invalid'); // Hapus tanda jika valid
                }

                if (!dataForm['alamat_instansi']) {
                    isValid = false; // Jika alamat instansi tidak diisi
                    $('#pekerjaanContainer textarea[name="alamat_instansi"]').addClass('is-invalid'); // Tandai input alamat instansi
                } else {
                    $('#pekerjaanContainer textarea[name="alamat_instansi"]').removeClass('is-invalid'); // Hapus tanda jika valid
                }
            }
    ...
    // Panggil fungsi secara berurutan untuk Tempat Tinggal
    ...
    function resetAlamatTinggal() {
        $('#alamat_tinggal').val(''); // Mengosongkan input alamat tinggal
    }

    // Event handler untuk Provinsi Tempat Tinggal (ketika user mengubah provinsi secara manual)
    ...
    // Panggil fungsi secara berurutan untuk Domisili
    ...
    // Fungsi untuk mengosongkan input alamat domisili
    function resetAlamatDomisili() {
        $('#alamat_domisili').val(''); // Mengosongkan input alamat domisili
    }

    // Event handler untuk Provinsi Domisili (ketika user mengubah provinsi secara manual)
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layout.alumniLayout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\tracking-alumni\tracking-alumni\resources\views/alumni/editbiodata.blade.php ENDPATH**/ ?>