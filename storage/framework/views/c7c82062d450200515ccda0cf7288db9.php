...
<script>
    ...
    const colorMap = {
        'Bekerja': '#254e7a', // Biru gelap
        'Wirausaha': '#5584b0', // Biru lebih terang
        'Pendidikan Lanjut': '#82c2e6', // Biru cerah
        'Belum Bekerja': '#cbe3ef', // Biru sangat cerah
        'Tidak Bekerja': '#f7f3ea' // Putih kekuningan
    };

    // Custom order untuk label
    const customOrder = ['Bekerja', 'Wirausaha', 'Pendidikan Lanjut', 'Belum Bekerja', 'Tidak Bekerja'];

    // Fungsi untuk membuat chart
    async function createChart() {
        const tahun = document.getElementById('tahun').value; // Mengambil nilai tahun
        const data = await fetchData(tahun);

        // Ambil status dan urutkan sesuai customOrder
        const statuses = Object.keys(data).sort((a, b) => customOrder.indexOf(a) - customOrder.indexOf(b));
        const statusCounts = statuses.map(status => data[status][tahun] || 0); // Ambil jumlah berdasarkan status

        const ctx = document.getElementById('alumniChart').getContext('2d');
        chart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: statuses, // Sumbu X berisi status
                datasets: [{
                    data: statusCounts, // Data jumlah sesuai status
                    backgroundColor: statuses.map(status => colorMap[status] || '#cccccc') // Warna sesuai status
                }],
            },
            options: {
                responsive: true,
                ...
                scales: {
                    ...
                    x: {
                        title: {
                            display: true,
                            text: 'Status Alumni' // Ganti teks sumbu X menjadi Status
                        }
                    }
                },
                plugins: {
                    legend: {
                        display: false, // Menyembunyikan legend (keterangan)
                    }
                }
            }
        });
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layout.alumniLayout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\tracking-alumni\tracking-alumni\resources\views/alumni/homealumni.blade.php ENDPATH**/ ?>