...
<?php else: ?>
<script>
    window.location.href = "<?php echo e(route('login')); ?>"; // Arahkan ke halaman login
</script>
<?php endif; ?>
...
<script>
    ...
    if (sidebar.classList.contains('sidebar-hidden')) {
        sidebar.classList.remove('sidebar-hidden'); // Tampilkan sidebar
        sidebar.classList.add('visible'); // Tambahkan kelas visible
        overlay.classList.add('visible'); // Tampilkan overlay
        content.classList.remove('content-collapsed'); // Atur konten agar bergeser ke kanan
        hamburgerButton.style.display = 'none'; // Sembunyikan tombol hamburger
    } else {
        sidebar.classList.add('sidebar-hidden'); // Sembunyikan sidebar
        sidebar.classList.remove('visible'); // Hapus kelas visible
        overlay.classList.remove('visible'); // Sembunyikan overlay
        content.classList.add('content-collapsed'); // Atur konten untuk penuh
        hamburgerButton.style.display = 'block'; // Tampilkan tombol hamburger
    }
}
    ...
    function previewPhoto() {
        ...
        if (file) {
            ...
            // Unggah secara otomatis setelah memilih gambar
            uploadPhoto();
        }
    }
        ...
        fetch(form.action, {
            ...
            .then(data => {
                ...
                if (data.success) {
                    Swal.fire({
                        title: 'Berhasil!',
                        text: data.message, // Tampilkan pesan dari respons JSON
                        icon: 'success',
                        confirmButtonText: 'OK'
                    }).then(() => {
                        // Reload halaman setelah menutup swal
                        location.reload();
                    });
                } else {
                    Swal.fire({
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    ...
                    // Redirect ke halaman logout setelah popup kedua
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layout.mainalumni', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\tracking-alumni\tracking-alumni\resources\views/layout/alumniLayout.blade.php ENDPATH**/ ?>